...
# use this line instead of the one above to get "klima" terms 
freq_weight <- textstat_frequency(dfm_klima, n=15, groups=dfm_weight_origin$origin)
...
ggsave("figure_PLOT.png", dpi=300, dev='png', height=10, width=15, units="in")
```
# Collocations
```{r}
...
p2000_coll_clean <- p_coll %>%
  dplyr::filter(!str_detect(collocation, de_stps),
                ...
                !str_detect(collocation, "00"),
                !str_detect(collocation, "30"),
                !str_detect(collocation, ">"),
                !str_detect(collocation, "newsletter"),
                !str_detect(collocation, "anmelden"),
                !str_detect(collocation, "\\+"),
                !str_detect(collocation, "40"),
                !str_detect(collocation, "81"),
                !str_detect(collocation, "erneuerbarer"))
...
c_coll <- textstat_collocations(contra2000, min_count=50)

# remove entries involving stopwords: NOT WORKING YET!
c2000_coll_clean <- c_coll %>%
  dplyr::filter(!str_detect(collocation, de_stps),
                !str_detect(collocation, en_stps),
                !str_detect(collocation, "="),
                !str_detect(collocation, "00"),
                ...
                !str_detect(collocation, "81"),
                !str_detect(collocation, "erneuerbarer"),
                !str_detect(collocation, "kommentare"),
                !str_detect(collocation, "\\|"),
                !str_detect(collocation, "\\^"),
                !str_detect(collocation, "grüßen"),
                !str_detect(collocation, "geehrter"),
                !str_detect(collocation, "globalen"))
#p2000_coll_clean
arrange(c2000_coll_clean, desc(count))
```
...
#corp_dfm_klima <- dfm_select(corp_dfm, pattern ="klima*")
...
# Plot estimated word keyness
textplot_keyness(result_keyness, margin=0.2, n=15) 
```
...
- DONE: Klimawörter Liste mit Group und Counts abspeichern -> welche Klimawörter gibt es wo und wie oft
