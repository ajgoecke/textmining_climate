...
```{r}
pro2000_sum <- summary(pro2000, n=100)
contra2000_sum <- summary(contra2000, n=100)
...
# Collocations
```{r}
p_coll <- textstat_collocations(pro2000, min_count=100)
arrange(p_coll, desc(count))
```
```{r}
# remove entries involving stopwords 
p2000_coll_clean <- p2000_coll %>%
  dplyr::filter(!str_detect(collocation, de_stopwords))
