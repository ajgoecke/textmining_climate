...
docvars(fff_de_corpus, "origin") <- textcat(fff_de_corpus)
...
```{r}
#try to filter out all german items per corpus for later use
ikem_corpus$language
```
...
## Corpus Cleaning
Todo: 
- extract english texts and delete from corpus (should probably be done in a step at the very beginning)
- idea: do this via trafilatura beforehand!
```{r}
head(klimarep_corpus)
```
```{r}
ikem_texts
```
...
```{r}
dfm <- dfm(ikem_corpus, remove=de_stopwords, remove_punct=TRUE, remove_numbers=TRUE)
