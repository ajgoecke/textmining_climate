...
```{r}
# optional to get "Sentences" Info for comparison
#sentinfo <- summary(contra_corpus)$Sentences

#to filter out long docs /// not working yet. there are still some smaller docs in there, why?
contra_sents <- corpus_subset(contra_corpus, summary(contra_corpus)$Sentences >= 20, drop_docid = TRUE)
pro_sents <- corpus_subset(pro_corpus, summary(pro_corpus)$Sentences >=20, drop_docid = TRUE)
```
