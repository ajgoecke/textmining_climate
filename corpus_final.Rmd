...
#ggsave("figure_PLOT.png", dpi=300, dev='png', height=10, width=10, units="in")
```
...
p <- ggplot(freqs) +
    ...
    ggtitle("Comparison 'Klima' Weighted Word Frequencies per Group") + 
...
## Collocations
```{r}
# transform stoplists for dplyr::filter function 
...
# remove entries involving stopwords:
p2000_coll_clean <- p_coll %>%
  dplyr::filter(!str_detect(collocation, de_stps),
                ...
                !str_detect(collocation, "\\|"),
                !str_detect(collocation, "OWLIT"))

#p2000_coll_clean
p2000_colls <- head(arrange(p2000_coll_clean, desc(count)), n=50)
...
c2000_coll_clean <- c_coll %>%
  dplyr::filter(!str_detect(collocation, de_stps),
                ...
                !str_detect(collocation, "erneuerbarer"),
                !str_detect(collocation, "\\|"),
                !str_detect(collocation, "et"),
                !str_detect(collocation, "\\´"),
                !str_detect(collocation, "dass"),
                !str_detect(collocation, "\\^"))

#p2000_coll_clean
c2000_colls <- head(arrange(c2000_coll_clean, desc(count)), n=50)
